...
This is the microcontroller code. It is uploaded to the Arduino Mega. It's purpose is to 
be the firmware for applicable sensor/actuator modules (SAMs). It has been written 
in such a way that each new type of sensor and actuator is its own module. Each 
sensor/actuator module must contain a class with the following methods:
    void begin(void) 
    String get(void) 
    String set(String instruction_code, int instruction_id, String instruction_parameter)
Each SAM must also be instantiated such that its modularity is prioritized. For example,
...
is up to the programmer to use their best judgement.
*/
