  ...
  while (1) {
    if (Serial.available()) { 
      if (Serial.read() == kAcknowledgeChar) { // await acknowledgement
        Serial.write(kAcknowledgeChar); // acknowledge acknowledgement
        not_connected_ = 0;
        return; // connected!
      }
    }
    if (millis() - start_time > kEstablishConnectionTimeout) {
      Serial.println("Did not establish connection");
      not_connected_ = 1;
      break; // timed out :(
    }
  }
...
void Communication::send(String outgoing_message) {
  ...
  if (not_connected_) {
    ...
    Serial.print(getPackedMessage(outgoing_message));
  }
}
...
String Communication::receive(void) { 
  ...
  if (not_connected_) {
    ...
    return getUnpackedMessage(incoming_message);
  }
}
